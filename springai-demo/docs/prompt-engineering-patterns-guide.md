...
大模型一旦训练完成，能力就固定了——你改不了它的参数。**你唯一能掌控的，是"怎么问"。** 同一个模型、
同一个任务，问法不同，质量可能天差地别：含糊的提问得到含糊的答案，精心设计的提问能稳定得到结构清晰、
准确可用的结果。把"怎么问"系统化、方法化，就是**提示词工程**——它是性价比最高的优化手段，不需要训练、
不需要算力，改几句话就能见效。
...
无论用哪种模式，写好提示词文本都有一套通用原则——它们是所有模式的"基本功"，先练好这些，再谈招式：

1. **明确、具体**：说清你要什么。"写一段产品介绍"不如"用 100 字、面向程序员、突出性能，介绍这款数据库"。
2. **给约束**：限定长度、语气、范围、受众。约束越清晰，输出越可控。
3. **指定输出格式**：要 JSON / 表格 / 列表就明说。需要**严格保证**格式时，用结构化输出（见
   [结构化输出指南](structured-output-guide.md)）从代码层面强制，而不只在提示词里"请求"。
4. **分隔指令与数据**：把"做什么"（指令）和"针对什么"（数据）用分隔符或标签隔开，避免模型混淆二者。
5. **给正例**：一两个示例往往胜过大段描述（这正是"多样本提示"的思路）。
6. **迭代**：第一版提示很少是最好的。看输出 → 调措辞 → 再试——提示词是"调"出来的，不是一次写成的。

下面 11 个模式，本质上就是这些原则在不同任务上的具体化与组合。
...
读完 11 个模式别只记住名字，关键是遇到任务知道选哪个。可以这样对号入座：

| 你的目标 | 首选模式 | 说明 |
|----------|----------|------|
| 简单分类、翻译、摘要 | 零样本 | 模型已会，直接下指令 |
| 要固定的输出格式 | 多样本（+ 结构化输出） | 给示例照着仿；要严格保证用 `BeanOutputConverter` |
| 设定固定身份/语气/规则 | 系统提示 / 角色提示 | 通用风格框架 vs 代入具体职业 |
| 一套提示复用到多场景 | 上下文提示 | 模板 + 参数注入 |
| 数学、逻辑、多步推理 | 思维链 | "一步步思考"，看中间步骤 |
| 推理任务要更高可靠性 | 自洽性 | 多跑几次 + 投票 |
| 开放性、需要权衡的决策 | 思维树 | 列候选 → 评估 → 深入 |
| 没思路、想创意发散 | 回退提示 / 自动提示词工程 | 先抽象再具体 / 让 AI 帮你想提示词 |
| 写代码 | 代码提示 | 讲清语言、输入输出、约束 |

> 模式可以**组合**：例如"多样本 + 思维链 + 低温"常一起用在"既要稳定格式又要推理"的任务上。
...
# 运行 main：依次执行全部 11 个模式 + 1 个「温度对照」附加演示
mvn compile exec:java -Dexec.mainClass="org.example.springai.PromptEngineeringPatterns"

# 也可在命令前内联 API Key（不写入文件）
DEEPSEEK_API_KEY=sk-xxx mvn compile exec:java -Dexec.mainClass="org.example.springai.PromptEngineeringPatterns"
```

> - 运行前需配置 DeepSeek API Key（参见 [API Key 配置](api-key-configuration.md)）。
> - 程序对每个模式都用 `runSafely` **容错执行**：长文本生成（如回退提示）偶发网络读超时，会**自动重试**，
>   单个失败也**不影响后续模式**，演示能完整跑到最后（个别模式重试仍失败会自动跳过并提示）。
> - **最推荐先看末尾的「温度对照」演示**：同一句提示在 0.0 与 1.0 各跑两次——0.0 两次几乎相同、
>   1.0 两次明显不同。这是亲身体会"温度"作用最直观的一步，也是其它模式选高/低温的依据。
...
5. 提示词里"请求"格式 ≠ "强制"格式：需要稳定的结构化数据时，配合
   [结构化输出指南](structured-output-guide.md) 用 `BeanOutputConverter` 从代码层面强制约束。
6. 建议运行 `PromptEngineeringPatterns` 并对照本文逐个观察，尤其留意末尾的「温度对照」演示，加深理解。
