...
同一个问题，换种问法，AI 的回答可能天差地别。**提示词工程，研究的就是"怎么问"。**

它有两个层面，缺一不可：

- **说什么**——提示词文本：你怎么组织语言、给不给例子、要不要设定角色。下面 11 个模式，主要练这一层。
- **怎么生成**——输出参数：用 `ChatOptions` 控制 AI 的生成方式，其中最重要的是 `temperature`（温度）。

> 温度记住这一句就够：**越低越确定、稳定；越高越随机、有创意。** 每个模式为什么这么设温度，都源于此。

下面 11 种被反复验证的"问法"，每种叫一个**模式**。每个都讲清楚 **是什么、什么时候用、怎么写**，并附真实
运行结果。看完，你就有了一套随取随用的提示词工具箱。
...
String options = chatClient.prompt("午餐列3个不同风格的选项(中/西/日)，各一句")
