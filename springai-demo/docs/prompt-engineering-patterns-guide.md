...
| 9 | 思维树 | Tree of Thoughts | 列候选→评估→深入（简化版） | 0.8→0.2 |
...
**定义**：思维树在思维链「单线推理」的基础上更进一步——先生成多个候选方案（分支），评估比较后选出最优的
一个，再沿它深入展开，整个过程形如一棵决策树。完整的 ToT 算法会在**多层树**上反复分支、打分、剪枝乃至
回溯；为便于入门，**本示例做了简化**，用「生成候选 → 评估选择 → 深入展开」三步直观演示其核心思想。
...
> **本示例是 ToT 思想的简化演示，请注意与完整算法的差别**：
> - 「分支」是用**一次调用**让模型列出 3 个候选（而非并行运行 3 条独立推理链）；
> - 「评估」选中 1 个，即相当于把其余分支「剪枝」掉；
> - 没有多层树、没有回溯。真实工程中的 ToT 会在多层树上反复分支、打分、回溯，复杂度高得多。
>
> 这样简化是为了让新手用三步看懂"分支→评估→深入"的核心思想，先建立直觉，再去理解完整算法。
...
> **本示例同样是简化演示**：只走到「生成 3 个变体 → 拿其中之一测试转 JSON」为止。完整的自动提示词工程
> 还会对每个变体**逐一打分、择优，甚至迭代优化**，形成"生成 → 评估 → 改进"的闭环。本示例省略了打分与
> 择优，重在展示"用 AI 生产提示词"这一核心思路。
