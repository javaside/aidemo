...
和 AI 对话，就像跟人沟通——同一件事，说得清不清楚、给没给背景，结果差别很大。**提示词工程，就是设计、
打磨"提示词"，引导 AI 产出更准确、更高质量回答的过程**（官方称它"既是一门艺术，也是一门科学"，要反复试）。

具体怎么做？无非两件事，是你手里两个可调的"旋钮"：
...
**像下棋一样思考**：先给出几个候选走法，评估挑出最好的，再**从它往前推演双方的后续几步**，看看走向如何。
关键就在最后这步"向前看分支"。

和上一个思维链对比着记：**思维链是"一条思路走到底"；思维树是"先铺几条思路、挑一条，再往前推演它的后续"。**

**什么时候用**：要走一步看几步、权衡不同走向的决策（下棋、博弈、多步规划）。

```java
// ① 生成几个候选走法（要多样 → 高温）
String moves = chatClient.prompt("""
        中国象棋开局阶段。给出3种不同开局走法，每种：
        (1)一句话描述 (2)战略思路 (3)强度打分(1-10)
        """)
...
// ③ 向前推演：从选定走法往后推双方各2步的分支变化
String projection = chatClient.prompt()
        .user(u -> u.text("基于选定走法，推演接下来双方各2步的可能变化，指出最有利的序列：\n{b}").param("b", best))
        .call().content();
```
...
```
① 三种开局：中炮(炮二平五,9分) / 飞相(8分) / 起马(8分)
② 选中：中炮——主动进攻、变化丰富
③ 向前推演：红炮二平五 → 黑马8进7 → 红马二进三……并给出两条分支(直车/挺七兵)各自的后续
```

💡 **价值在第③步"向前推演分支"：不是选完就完事，而是顺着选择往后看几步、再定最优走向——这才是"树"。**
