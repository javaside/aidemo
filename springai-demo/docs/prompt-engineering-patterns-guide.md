...
名字唬人，**说白了就是：让 AI 帮你生成"要发给 AI 的文字"**，省得自己一条条手写。

**这个例子想说明的事**：你在做一个下单功能，要把用户的话解析成订单。可用户的说法五花八门，你没法手写
几十条来测。于是让 AI **自动生成一批不同说法**当测试数据，再看你的功能是不是每条都能正确解析。

```java
// 第一步：让 AI 把同一个需求，自动写成 3 种不同说法（= 自动造测试输入）
String variants = chatClient.prompt("把\"买一件蓝色T恤 M码\"换成3种不同的说法")
        .options(ChatOptions.builder().temperature(0.9).build())   // 要多样，高温
        .call().content();

// 第二步：把这 3 种说法逐一丢给下单功能，看是否都能正确解析成订单
chatClient.prompt()
...
```
AI 自动写出 3 种说法：①我要M码蓝色T恤 ②想买蓝T恤尺码M ③帮我拿件蓝T恤M号
三种都被正确解析成同一个订单：{"item":"T恤","color":"蓝色","size":"M"}
```

**什么时候用**：想批量造测试数据，或自己拿不准措辞、让 AI 帮你想说法时。

💡 **第二步转 JSON 只是"验收"——三种说法都解析成同一个订单，说明你的功能对各种说法都扛得住。**
